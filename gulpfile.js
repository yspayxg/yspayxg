...
// 合并 css 并压缩（由于覆盖问题，css存在先后顺序）
gulp.task('package-css', function() {
  return gulp.src(['public/css/weui.css', 'public/css/swipe.css', 'public/css/book.css'])
    .pipe(concat('book.css'))//将上面三个css文件合并为一个CSS文件book.css
    .pipe(minifyCss())//将上面的book.css文件进行压缩
    .pipe(rename('book.min.css'))//将压缩后的book.css重命名，即意味着改文件是压缩过的
    .pipe(gulp.dest('public/build'));//将book.min.css放在build文件夹
});
