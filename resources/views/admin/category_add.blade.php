...
<form action="" method="post" class="form form-horizontal" id="form-category-add">
  ...
  <div class="row cl">
    ...
    <div class="formControls col-5">
      <input type="number" class="input-text" value="0" placeholder="" name="category_no"  >
    </div>
    <div class="col-4"> </div>
  </div>
  <div class="row cl">
  ...
  $("#form-category-add").Validform({
    ...
    callback:function(form){
      ...
      $("#form-category-add").ajaxSubmit({
          ...
          url: '/dashboard/laravel52/public/admin/service/category/add', // 需要提交的 url
          dataType: 'json',
          data: {
            name: $('input[name=name]').val(),
            category_no: $('input[name=category_no]').val(),
            parent_id: $('select[name=parent_id] option:selected').val(),
          preview: ($('#preview_id').attr('src')!='/dashboard/laravel52/public/admin/images/icon-add.png'?$('#preview_id').attr('src'):''),
            _token: "{{csrf_token()}}"
          },
